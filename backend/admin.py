...
def list_users():
    ...
    return jsonify({
        'users': [
            {**u.to_dict(), 'paper_count': len(u.papers)}
            for u in users
        ]
    })
...
@jwt_required()
def list_all_papers():
    """List all papers across all users."""
    if not _require_admin():
        return jsonify({'error': 'Admin access required'}), 403

    papers = Paper.query.join(User).order_by(desc(Paper.updated_at)).all()
    return jsonify({
        'papers': [
            {
                **p.to_dict(),
...
def get_usage_stats():
    ...
    ).group_by(ApiUsageLog.endpoint).order_by(desc('tokens')).all()
    ...
    return jsonify({
        ...
        'by_endpoint': [
            {'endpoint': e, 'calls': int(c), 'tokens': int(t or 0)}
            for e, c, t in by_endpoint
        ],
        'daily': [
            {'date': str(d), 'calls': int(c), 'tokens': int(t or 0)}
            for d, c, t in daily
        ],
        'per_user': [
